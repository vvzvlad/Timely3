...
// weather_icons); this file pins WHICH FIELD OWNS WHICH BYTE, for the four
...
// every size and every asserted offset intact while handing a byte of the
// persisted blob to a different field, so existing users' settings are silently
// misread. That applies to a field sitting directly before an asserted one too:
// offsetof(theme_mode) == 24 says nothing about which field owns byte 23 — it
...
//
// The sizes (25 / 244) and the offsets the header already asserts are NOT
...
UTEST(settings_layout, persist_interior_fields_pinned) {
  ASSERT_EQ(18u, (unsigned)offsetof(persist, strftime_format_reserved));
  ASSERT_EQ(4u, (unsigned)sizeof(((persist *)0)->strftime_format_reserved));
  ASSERT_EQ(23u, (unsigned)offsetof(persist, theme));
}
